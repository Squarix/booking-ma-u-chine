...
export default class Room {
  ...
  updateRoom = async (roomId, params, currentUser) => {
    try {
      const updated = await this.roomModel.update(params, {where: {id: roomId, userId: currentUser.id}});
      return {status: 200, message: 'Updated', updated};
    } catch (e) {
      console.log(e)
  ...
  roomBookings = roomId => this.bookingModel.findAll({
    where: {
      roomId, status: {
        [Op.not]: 'declined'
      }
  ...
  getFilters = () => this.filterModel.findAll({attributes: ['id', 'filter']})
}
